...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action']) && $_POST['action'] === 'create_checkout') {
    ...
    try {
        // Start a transaction to save pending order
        $connection->begin_transaction();

        // Save to pending_orders
        $stmt = $connection->prepare("INSERT INTO pending_orders (customer_id, total_amount, status) VALUES (?, ?, 'pending')");
        $stmt->bind_param("id", $customer_id, $grand_total);
        $stmt->execute();
        $pending_order_id = $connection->insert_id;

        // Save to pending_order_items
        $stmt = $connection->prepare("INSERT INTO pending_order_items (pending_order_id, item_name, type, price, quantity) VALUES (?, ?, ?, ?, ?)");
        foreach ($cart_items as $item) {
            $stmt->bind_param("issdi", $pending_order_id, $item['item_name'], $item['type'], $item['price'], $item['quantity']);
            $stmt->execute();
        }
        ...
        $_SESSION['pending_order_id'] = $pending_order_id; // Store for later use
...
// Handle payment confirmation and save final order
if (isset($_GET['session_id'])) {
    try {
        ...
        if ($session->payment_status === 'paid') {
            ...
            $stmt = $connection->prepare("INSERT INTO orders (customer_id, stripe_session_id, total_amount, status) VALUES (?, ?, ?, 'paid')");
            $stmt->bind_param("isds", $customer_id, $_GET['session_id'], $grand_total);
            $stmt->execute();
            $order_id = $connection->insert_id;

            // Save to order_items table
            $cart_items = $_SESSION['cart_items'];
            $stmt = $connection->prepare("INSERT INTO order_items (order_id, item_name, type, price, quantity) VALUES (?, ?, ?, ?, ?)");
            foreach ($cart_items as $item) {
                $stmt->bind_param("issdi", $order_id, $item['item_name'], $item['type'], $item['price'], $item['quantity']);
                $stmt->execute();
            }
            ...
            if (isset($_SESSION['pending_order_id'])) {
                ...
                $stmt = $connection->prepare("UPDATE pending_orders SET status = 'completed' WHERE id = ? AND customer_id = ?");
                $stmt->bind_param("ii", $pending_order_id, $customer_id);
                $stmt->execute();
            }
            ...
            // Mark pending order as failed if payment fails
            if (isset($_SESSION['pending_order_id'])) {
                $pending_order_id = $_SESSION['pending_order_id'];
                $stmt = $connection->prepare("UPDATE pending_orders SET status = 'failed' WHERE id = ? AND customer_id = ?");
                $stmt->bind_param("ii", $pending_order_id, $customer_id);
                $stmt->execute();
            }
            $error_message = "Payment failed or not completed.";
        }
    } catch (Exception $e) {
        $connection->rollback();
